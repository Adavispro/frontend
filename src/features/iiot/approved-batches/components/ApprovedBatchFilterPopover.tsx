...
export interface ApprovedFilterValues {
  productCode: string;
  productName: string;
  batchNo: string;
...
export interface ApprovedBatchFilterPopoverProps {
  isOpen: boolean;
  onClose: () => void;
  filters: ApprovedFilterValues;
  onChange: (field: keyof ApprovedFilterValues, value: string) => void;
  onReset: () => void;
...
export default function ApprovedBatchFilterPopover({
  isOpen,
  onClose,
  filters,
  onChange,
  ...
  availableLotNos,
  activeFilterCount,
}: ApprovedBatchFilterPopoverProps) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label="Approved batches filter menu"
    >
      {/* Popover Header */}
        ...
        <div className="flex items-center gap-2">
          {activeFilterCount > 0 && (
            <button
              type="button"
              onClick={onReset}
              className="text-xs font-semibold text-emerald-700 hover:text-emerald-800 transition px-2 py-1 bg-emerald-50 hover:bg-emerald-100 rounded-lg flex items-center gap-1"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 text-xs">
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 font-mono transition"
          >
        ...
        <div className="space-y-1">
          ...
          <select
            ...
            className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition"
          >
        ...
        {/* 6. Date Range: From Date */}
        <div className="space-y-1">
          <label className="block text-[11px] font-semibold text-slate-700 flex items-center gap-1">
            <CalendarBlank className="h-3.5 w-3.5 text-slate-400" />
            <span>Approved Date From</span>
          </label>
          <input
            type="date"
        ...
        {/* 7. Date Range: To Date */}
        <div className="space-y-1 sm:col-span-2">
          <label className="block text-[11px] font-semibold text-slate-700 flex items-center gap-1">
            <CalendarBlank className="h-3.5 w-3.5 text-slate-400" />
            <span>Approved Date To</span>
          </label>
          <input
            type="date"
