...
export async function GET(request: Request) {
  ...
  try {
    const { response, result } = await serverApiClient<Groups>(
      SERVER_API_CONFIG.mdmServiceUrl,
      withQuery(API_ENDPOINTS.masterManagement.userGroups, {
    ...
    return NextResponse.json({ ...result, data: parsedGroups.data });
  } catch {
    return NextResponse.json(
      {
        success: false,
...
export async function POST(request: Request) {
  ...
  const parsedRequest = createGroupRequestSchema.safeParse(body);
  ...
  try {
    ...
    >(
      SERVER_API_CONFIG.mdmServiceUrl,
      API_ENDPOINTS.masterManagement.userGroups,
      {
        method: "POST",
