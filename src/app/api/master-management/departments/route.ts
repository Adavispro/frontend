...
export async function GET(request: Request) {
  ...
  try {
    const { response, result } = await serverApiClient<Departments>(
      SERVER_API_CONFIG.mdmServiceUrl,
      withQuery(API_ENDPOINTS.masterManagement.departments, {
    ...
    return NextResponse.json({ ...result, data: parsedDepartments.data });
  } catch {
    return NextResponse.json(
      {
        success: false,
...
export async function POST(request: Request) {
  ...
  const parsedRequest = createDepartmentRequestSchema.safeParse(body);
  ...
  try {
    ...
    >(
      SERVER_API_CONFIG.mdmServiceUrl,
      API_ENDPOINTS.masterManagement.departments,
      {
        method: "POST",
    ...
    return NextResponse.json(
      { ...result, data: parsedDepartment.data },
