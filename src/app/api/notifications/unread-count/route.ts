...
export async function GET(request: Request) {
  ...
  const upstreamPath = `/api/v1/notifications/unread-count${requestUrl.search}`;
  ...
  try {
    const { response, result } = await serverApiClient<{ unreadCount: number }>(
